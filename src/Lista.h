...
template<class T> class Lista {
    ...
    public:
        ...
        /*
         * post: remueve datoARemover de la Lista
         */
        void removerPorDato(T datoARemover);

        bool estaElDato(T datoAComparar);
...
template <class T> void Lista<T>::removerPorDato(T datoARemover){

		Nodo<T>* nodoActual = this->primero;
		unsigned int posicion = 1;

		while(nodoActual->obtenerDato() != datoARemover && posicion <= this->contarElementos()){
			nodoActual = nodoActual->obtenerSiguiente();
			posicion ++;
		}

		if(datoARemover == nodoActual->obtenerDato()){

			if(posicion == 1){
				this->primero = nodoActual->obtenerSiguiente();
			}
...
template <class T> bool Lista<T>::estaElDato(T datoAComparar){
	bool encontro = false ;
	Nodo<T>* nodoActual = this->primero;
	unsigned int posicion = 1;

	while(nodoActual->obtenerDato() != datoAComparar && posicion <= this->contarElementos()){
		nodoActual = nodoActual->obtenerSiguiente();
		posicion++;
	}

	if(datoAComparar == nodoActual->obtenerDato()){
		encontro = true;
	}
